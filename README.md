...
- **使用 CPU**：仅当前文件改用 CPU，不修改队列中其他文件的全局后端设置；
- **继续当前 GPU / Auto**：继续 Auto / Vulkan。GPU 长录音仍按现有策略分段处理；若某个分段出现明确 Vulkan 显存/设备内存错误，会自动回退 CPU；
- **停止队列**：停止整个队列，当前文件保持“等待中”，以后可继续；
- **10 秒内不做任何选择**：自动按“使用 CPU”处理当前文件。
